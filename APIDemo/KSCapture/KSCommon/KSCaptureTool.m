...
+ (NSString *)videoFilePath {
    ...
    NSString *videosPath = [docPath() stringByAppendingString:@"/videos"];
    NSString* mp4FileName = [[format stringFromDate:[NSDate date]] stringByAppendingString:@".mp4"];
    NSString *path = [videosPath stringByAppendingPathComponent:mp4FileName];
    return path;
}
...
+(void)scanAlltheMP4FilewWithComplete:(void (^)(NSArray *))complete {
  __block  NSMutableArray *items = [NSMutableArray new];
   NSArray *subPaths = [[NSFileManager defaultManager] subpathsAtPath:docPath()];
    [subPaths enumerateObjectsUsingBlock:^(NSString *pathValue, NSUInteger idx, BOOL * _Nonnull stop) {
        [items addObject:[docPath() stringByAppendingPathComponent:pathValue]];
    }];
    complete(items);
}
