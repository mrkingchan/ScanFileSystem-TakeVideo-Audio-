//
//  SGScanningQRCodeVC.h
//  SGQRCodeExample
//
//  Created by Sorgle on 16/8/25.
//  Copyright © 2016年 Sorgle. All rights reserved.
//
//  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //
//
//  - - 如在使用中, 遇到什么问题或者有更好建议者, 请于kingsic@126.com邮箱联系 - - - - - //
//  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//  - - GitHub下载地址 https://github.com/kingsic/SGQRCode.git - - - - - - - - - //
//
//  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //


#import <UIKit/UIKit.h>
#import "YQRootViewController.h"

@protocol SGScanningQRCodeVCDelegate;


@interface SGScanningQRCodeVC : YQRootViewController

@property (nonatomic,assign)id<SGScanningQRCodeVCDelegate> delegate;

@end

@protocol SGScanningQRCodeVCDelegate <NSObject>

-(void)scanSuccessBarcodeJump:(NSString*)str;

@end
